...
def ddbb_create_intermediate(variant_dir, coverage_dir, min_freq_discard=0.8, min_alt_dp=14, only_snp=False, samples=False):
            ...
            if name == "snps.all.ivar.tsv":
                ...
                if samples == False:
                    ...
                    dfv = import_tsv_variants(
                        filename, sample, min_total_depth=20, min_alt_dp=10, only_snp=only_snp)
...
if __name__ == '__main__':
    ...
    if args.only_compare == False:
        ...
        compare_snp_matrix_DPU_intermediate = full_path_compare + \
            ".revised_DPU.tsv"

        # Create intermediate

        recalibrated_snp_matrix_intermediate = ddbb_create_intermediate(
            out_variant_dir, out_stats_coverage_dir, min_freq_discard=0.8, min_alt_dp=15, only_snp=False, samples=sample_list)
        ...
        # Convert decimals to 0 - 1 (>=0.8)
        df = recalibrated_snp_matrix_intermediate
        ...
        print("INDELs excluded")
        # Create a column with only positions
        df["REF"] = df.Position.str.split("|").str[-1]
        df["ALT"] = df.Position.str.split("|").str[-3]
        df = df[(df['REF'].str.len() < 2) & (df['ALT'].str.len() < 2)]

        # Delete unnecessary columns
        df.drop(columns=["REF", "ALT"], inplace=True)

        # Transform decimals
        # Specify the column names with decimals (excluding the first column)
        decimal_columns = df.columns[1:]

        # Convert values >= 0.8 to 1 and others to 0
        df[decimal_columns] = df[decimal_columns].applymap(lambda x: 1 if x >= 0.8 else 0)
        df.to_csv(compare_snp_matrix_DPU_intermediate, sep="\t", index=False)
