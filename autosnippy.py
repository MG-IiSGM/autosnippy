...
from misc import extract_sample, check_create_dir, extract_read_list, file_to_list, obtain_group_cov_stats, check_reanalysis, remove_low_quality, obtain_overal_stats, \
    fastqc_quality, \
    run_snippy, run_snippy_core, extract_indels, merge_vcf, create_bamstat, create_coverage, extract_close_snps, identify_uncovered, remove_position_from_compare, \
    vcf_to_ivar_tsv, import_VCF4_core_to_compare, \
    mash_screen, kraken, \
    annotate_snpeff, rename_reference_snpeff, user_annotation, user_annotation_aa

from compare_snp_autosnippy import ddbb_create_intermediate, remove_bed_positions, recalibrate_ddbb_vcf_intermediate, remove_position_range, extract_lowcov, revised_df, extract_bed_positions, ddtb_compare

from arguments import get_arguments

"""
── Information on the bioinformatics workflow ─────────────────────────────

Institution: IiSGM
Author: Sergio Buenestado-Serrano (sergio.buenestado@gmail.com)
Version update: v2.0 on 09 Jun 2026
Created: 22 Feb 2021 by Pedro J. Sola (pedroscampoy@gmail.com)
...
def main():
    ...
    logger.info("\n\n" + BLUE + BOLD + "STARTING PIPELINE IN GROUP: " + NORMAL + group_name + END_FORMATTING)
    ...
    logger.info("\n%d NEW samples will be analysed: %s" % (len(new_samples), ",".join(new_samples)))


    # ── DECLARE FOLDERS & KEY FILES ──────────────────────────────────────────
    # Output related
    ...
    for r1_file, r2_file in zip(r1, r2):
        ...
        if sample in sample_list_F:
            ...
            if sample in new_samples:
                ...
                logger.info("\n" + WHITE_BG + "STARTING SAMPLE: " + sample + " (" + sample_number + "/" + sample_total + ")" + " (" + new_sample_number + "/" + new_sample_total + ")" + END_FORMATTING)
            else:
                logger.info("\n" + WHITE_BG + "STARTING SAMPLE: " + sample + " (" + sample_number + "/" + sample_total + ")" + END_FORMATTING)

            output_final_vcf = os.path.join(sample_variant_dir, 'snps.all.ivar.tsv')
            ...
            if not os.path.isfile(output_final_vcf):
                ...
                if os.path.isfile(output_vcf_sub) and os.path.isfile(output_vcf):
                    ...
                    run_snippy(r1_file, r2_file, reference, out_variant_dir, sample, threads=args.threads, minqual=10, minfrac=0.1, mincov=1)
            ...
            if args.mash_db != False:
                if os.path.isfile(output_species):
                    ...
                    mash_screen(r1_file, out_species_dir, r2_file=r2_file, winner=True, threads=args.threads, mash_database=args.mash_db)

                    # Name the columns of the mash output and sort them in descending order by identity
                    output_sort_species = pd.read_csv(output_species, sep='\t', header=None, names=['Identity', 'Share-hashes', 'Median-multiplicity', 'p-value', 'ID accession', 'Organism']).sort_values(by=['Identity'], ascending=False)
                    output_sort_species.to_csv(output_species, sep='\t', index=None)
            else:
                logger.info(YELLOW + BOLD + "No Mash database suplied, skipping specie assignation in group " + group_name + END_FORMATTING)


            # ── Create Stats & Quality Filters ───────────────────────────────────────
    ...
    uncovered_samples = remove_low_quality(output, cov20=args.coverage20, unmapped_per=args.unmapped, min_hq_snp=args.min_snp, type_remove='Uncovered')

    if len(uncovered_samples) > 1:
        logger.info(GREEN + "Uncovered samples: " + (",").join(uncovered_samples) + END_FORMATTING)
    else:
    ...
    if args.core:
        ...
        logger.info(GREEN + "Running snippy-core " + group_name + END_FORMATTING)

        run_snippy_core(out_variant_dir, out_core_dir, reference)

        logger.info(GREEN + "Adapting core-snp to compare format " + group_name + END_FORMATTING)

        core_vcf_file = os.path.join(out_core_dir, "core.vcf")
        core_vcf_file_adapted = os.path.join(out_core_dir, "core.vcf.adapted.tsv")
        core_vcf_file_removed = os.path.join(out_core_dir, "core.vcf.adapted.final.tsv")
        ...
        logger.info(GREEN + "Obtaining clustered positions " + group_name + END_FORMATTING)

        close_positions_list = extract_close_snps(core_vcf_df_adapted, snps_in_10=1)

        logger.info(GREEN + "Obtaining uncovered positions " + group_name + END_FORMATTING)

        uncovered_list = identify_uncovered(out_stats_coverage_dir, min_coverage=10, nocall_fr=0.5)

        logger.debug('Clustered positions in core SNP:\n{}'.format((",".join([str(x) for x in close_positions_list]))))
        logger.debug('Uncovered positions in all samples:\n{}'.format((",".join([str(x) for x in uncovered_list]))))

        to_remove_list = close_positions_list + uncovered_list

        remove_df = remove_position_from_compare(core_vcf_df_adapted, to_remove_list)
        remove_df.to_csv(core_vcf_file_removed, sep="\t", index=False)

        ddtb_compare(core_vcf_file_removed, distance=args.distance)


    # ── Annotation with SNPeff & User ────────────────────────────────────────
            ...
            for name in files:
                if name == 'snps.all.vcf':
                    ...
                    if os.path.isfile(out_annot_file):
                        ...
                        annotate_snpeff(chrom_filename, out_annot_file, database=args.snpeff_database)
    ...
    if args.remove_bed:
        recalibrated_snp_matrix_intermediate = remove_bed_positions(recalibrated_snp_matrix_intermediate, args.remove_bed, full_path_compare)

    recalibrated_snp_matrix_intermediate.write_csv(compare_snp_matrix_recal_intermediate, separator="\t")

    logger.info(GREEN + BOLD + "[COMPARE] " + NORMAL + f"Intermediate matrix saved to: {compare_snp_matrix_recal_intermediate}" + END_FORMATTING + '\n')
    ...
    if args.extract_bed:
        ...
        annotated_snps_final = extract_bed_positions(recalibrated_snp_matrix_mpileup, args.extract_bed, full_path_compare)
